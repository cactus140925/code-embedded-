...
// HTTP (sensor)
const char* sensorDataURL = "http://172.19.248.29:1880/dht11";

// MQTT
const char* mqtt_server = "172.19.248.29";
...
if (buzzerActive && millis() - buzzerStartTime >= 5000) {
  ...
  if (now - lastSensorPrint >= printInterval) {
    Serial.printf("🌡 Nhiệt độ: %.1f°C | 💧 %.1f%% | 🌱 %d%% | 🔆 Ánh sáng: %d%%\n", data.temperature, data.humidity, soilMoisture, lightValue);
  ...
  if (now - lastSendTime >= sendInterval) {
    ...
    if (!isnan(data.temperature) && !isnan(data.humidity)) {
      ...
      if (code > 0) Serial.printf("📤 Gửi HTTP cảm biến OK | Phản hồi: %d\n", code);
